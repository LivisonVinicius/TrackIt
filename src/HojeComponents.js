...
export default function HojeComponents() {
  ...
  const habitComponent = habitos.map((habito) => (
    <Habito background={habito.done ? "#8fc549" : "#EBEBEB"} current={habito.done ? "#8fc549" : "#666666"} record={habito.currentSequence===habito.highestSequence && habito.highestSequence!==0?"#8fc549" : "#666666"}>
      <h3>{habito.name}</h3>
      <h4 className="current">Sequência atual:<span>{habito.currentSequence} {habito.currentSequence>1?"dias":"dia"}</span></h4>
      <h4 className="record">Seu recorde:<span>{habito.highestSequence}</span></h4>
